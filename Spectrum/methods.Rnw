...
\par
<<echo=FALSE, message=FALSE, results='asis', warning=FALSE, fig.width=5, fig.height=4.5>>=
library(haven)
library(ggplot2)
library(dplyr)

df <- read_dta("Pippa.dta") %>% 
  select(Year, FHouse, polity2)
df[] <- lapply(df, unclass)

ds <- df %>% 
  filter(!is.na(FHouse), !is.na(polity2)) %>% 
  group_by(Year) %>% 
  mutate(Polity = mean(polity2 + 20)) %>% 
  mutate(FreedomHouse = mean(FHouse)*5)

ggplot(ds) +
  geom_smooth(aes(x = Year, y = FreedomHouse, color = "Freedom House")) +
  geom_smooth(aes(x = Year, y = Polity, color = "Polity IV")) +
  ylab("Scaled Democractic Score") +
  ggtitle("Scaled Democratic Scores of Freedom House and Polity IV") +
...
<<breakdown, echo=FALSE, message=FALSE, results='asis'>>=
library(readr)
library(stargazer)
df <- read_csv("SanctionsFinal.csv") %>% 
  select(polity2, Sanction = sanctionsl1) %>% 
  filter(!is.na(Sanction)) %>% 
  group_by(Sanction) %>% 
  summarise(Regime = round(mean(polity2), 3))

stargazer(df, summary = F, rownames = FALSE, title = "Average Polity IV Score by Dichotomous Lagged Sanction")
@
\par
...
l1 <- range(d1$polity2[which(d1$deliniation==1)])
l2 <- range(d1$polity2[which(d1$deliniation==2)])
l3 <- range(d1$polity2[which(d1$deliniation==3)])
l4 <- range(d1$polity2[which(d1$deliniation==4)])
l5 <- range(d1$polity2[which(d1$deliniation==5)])

c1 <- range(d1$polity2[which(d1$cluster==1)])
c2 <- range(d1$polity2[which(d1$cluster==2)])
c3 <- range(d1$polity2[which(d1$cluster==3)])
c4 <- range(d1$polity2[which(d1$cluster==4)])
c5 <- range(d1$polity2[which(d1$cluster==5)])

t1 <- cbind(l1, l2, l3, l4, l5)
t2 <- cbind(c1, c2, c3, c4, c5)

set1 <- as.data.frame(apply(t1,2,paste,collapse=" to "))  
set2 <- as.data.frame(apply(t2,2,paste,collapse=" to "))  

d2 <- cbind(set1, set2)

names(d2)[1] <- "Theoretical"
names(d2)[2] <- "Non-parametric"
...
stargazer(d2, summary = F, rownames = FALSE, title = "Breakdown of Theoretical and Non-parametric Delineation of Democracy Scores")
@
...
load("C:/Users/Darin/Documents/sanctionsbackslide/Spectrum/Table1.Rdata")
table1 <- table1[,-7]
names(table1)[names(table1)== c("p.val")] <- c("P-Value")
names(table1)[names(table1)== c("Original.n")] <- c("Cases")
names(table1)[names(table1)== c("Matched.n")] <- c("Matched/Treated Cases")
names(table1)[names(table1)== c("AI.SE")] <- c("SE")

stargazer(table1, summary = F, rownames = FALSE, title = "Estimated Effect of Economic Sanctions on Democracy", digits = 2)
@
...
<<echo=FALSE, results='asis', message=FALSE, warning=FALSE>>=
load("C:/Users/Darin/Documents/sanctionsbackslide/Spectrum/Table2.Rdata")
table2 <- table2[,-7]
names(table2)[names(table2)== c("p.val")] <- c("P-Value")
names(table2)[names(table2)== c("Original.n")] <- c("Cases")
names(table2)[names(table2)== c("Matched.n")] <- c("Matched/Treated Cases")
names(table2)[names(table2)== c("AI.SE")] <- c("SE")

stargazer(table2, summary = F, rownames = FALSE, title = "Estimated Effect of Lagged Economic Sanctions on Democracy", digits = 2)
@
\par
...
\par
<<echo=FALSE, results='asis', message=FALSE, warning=FALSE>>=
load("C:/Users/Darin/Documents/sanctionsbackslide/Spectrum/Table5.Rdata")
table5 <- table5[,-7]
names(table5)[names(table5)== c("p.val")] <- c("P-Value")
names(table5)[names(table5)== c("Original.n")] <- c("Cases")
names(table5)[names(table5)== c("Matched.n")] <- c("Matched/Treated Cases")
names(table5)[names(table5)== c("AI.SE")] <- c("SE")

stargazer(table5, summary = F, rownames = FALSE, title = "Estimated Effect of (Lagged) U.S. and U.N. Economic Sanctions on Democracy", digits = 2)
@
...
<<echo=FALSE, results='asis', message=FALSE, warning=FALSE>>=
load("C:/Users/Darin/Documents/sanctionsbackslide/Spectrum/Table6.Rdata")
table6 <- table6[,-7]
names(table6)[names(table6)== c("p.val")] <- c("P-Value")
names(table6)[names(table6)== c("Original.n")] <- c("Cases")
names(table6)[names(table6)== c("Matched.n")] <- c("Matched/Treated Cases")
names(table6)[names(table6)== c("AI.SE")] <- c("SE")

stargazer(table6, summary = F, rownames = FALSE, title = "Estimated Effect of (Lagged) U.S. Economic Sanctions on High and Low Ranges of Democracy", digits = 2)
@
\par
